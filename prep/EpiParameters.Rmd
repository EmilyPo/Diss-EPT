...
## Multivariate 
```{r acts.full, echo=F}
acts.full <- svyglm(sex4wk ~ comb.age + ptype + duration, family = "poisson", design=svydat) 
summary(acts.full)
```

## Output for Epimodel
```{r acts, echo=F}
acts.mod <- glm(sex4wk ~ comb.age + ptype + duration, 
                family = "poisson", 
                weights=data2$weight, 
                data=data2) 

#rates2 <- unname(predict(actsMonthGlm2, data2, type = "response"))/4
...
### Duration 
```{r condoms-duration}
c.svy.dur <- svyglm(formula = cbind(cond4wk, sex4wk-cond4wk) ~ duration, 
               family=binomial, design=svydat) 
summary(c.svy.dur)
```
## Output for EpiModel
...
```{r, echo=F}
condom.mod <- glm(formula = cbind(cond4wk, sex4wk-cond4wk) ~ comb.age + ptype, 
